...
extern CoreSurfaceBufferRef __screenSurfaceGBC;
extern void sound_mix2(short *sndbuf,int len);

extern void updateGBCScreen();
extern int iphone_main_gbc(char* filename);

/* app Resources */
extern char __fileNameTempSave[512];
extern byte IS_DEBUG;
byte IS_CHANGING_ORIENTATION;
unsigned long cPad1GBC;

extern AQCallbackStruct in;
    ...
    AudioQueueBufferRef outQB)
{
	...
	if (inData->frameCount > 0) {
		...
		sound_mix2(coreAudioBuffer, (inData->frameCount * 4) / 2);
...
//    DLog("app_SoundOpen()");
    ...
    err = AudioQueueNewOutput(&in.mDataFormat,
                      ...
                      &in.queue);
   if (err) {
//     DLog("AudioQueueNewOutput err %d\n", err);
   }
   ...
   for (i=0; i<AUDIO_BUFFERS; i++) {
      ...
      if (err) {
//	DLog("AudioQueueAllocateBuffer[%d] err %d\n",i, err);
      }
   ...
   soundInit = 1;
//   DLog("app_QueueSample.AudioQueueStart");
...
//    DLog("app_CloseSound.AudioQueueDispose()");
...
int app_LoadPreferences() {
    ...
    IS_DEBUG = 0;
#endif
//    DLog("app_LoadPreferences: Loading preferences");
    return (r) ? 0 : -1;
}
