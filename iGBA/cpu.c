...
#define check_for_interrupts()                                                \
  ...
  {                                                                           \
    ...
    arm_update_pc();                                                          \
    set_cpu_mode((cpu_mode_type)MODE_IRQ);                                                   \
    goto arm_loop;                                                            \
  }                                                                           \
    ...
    {                                                                         \
      ...
      {                                                                       \
        ...
        default:                                                              \
          ...
          set_cpu_mode((cpu_mode_type)MODE_SUPERVISOR);                                      \
          break;                                                              \
      }                                                                       \
      break;                                                                  \
    }                                                                         \
...
void print_flags()
{
  ...
  debug_screen_newline(1);
  debug_screen_printf(
   " C: %08x  S: %08x  mode: %s N: %d  Z: %d  C: %d  V: %d  ",
   cpsr, spsr[reg[CPU_MODE]],
   cpu_mode_names[reg[CPU_MODE]], (cpsr >> 31) & 0x01, (cpsr >> 30) & 0x01, (cpsr >> 29) & 0x01,
   (cpsr >> 28) & 0x01);
...
void init_cpu()
{
  ...
  memset((void *)rom_translation_cache, 0, ROM_TRANSLATION_CACHE_SIZE);
  rom_translation_ptr = rom_translation_cache;
  memset(ram_translation_cache, 0, RAM_TRANSLATION_CACHE_SIZE);
  ram_translation_ptr = ram_translation_cache;
  memset(bios_translation_cache, 0, BIOS_TRANSLATION_CACHE_SIZE);
